...
#define FORMAT "%f"
	...
	fprintf(file, "\tspin1x="FORMAT"\n\tspin1y="FORMAT"\n\tspin1z="FORMAT"\n", sim.spin1x,
		sim.spin1y, sim.spin1z);
	fprintf(file, "\tspin2x="FORMAT"\n\tspin2y="FORMAT"\n\tspin1z="FORMAT"\n", sim.spin2x,
		sim.spin2y, sim.spin2z);
	fprintf(file, "\tfL="FORMAT"\n\tfF="FORMAT"\n", sim.f_lower, sim.f_final);
	fprintf(file, "\td="FORMAT"\n\tdt="FORMAT"\n", sim.distance, ppn.deltaT);
	fprintf(file, "}\n\n");
}

int write_confuse(char const *filename, Parameters *what, SimInspiralTable default_siminsp,
...
int parse_confuse(char const *filename, Parameters *out) {
	...
	cfg_opt_t params[] = { CFG_FLOAT("m1", 1.0, CFGF_NONE), CFG_FLOAT("m2", 1.0, CFGF_NONE),
		CFG_FLOAT("spin1x", 0.1, CFGF_NONE), CFG_FLOAT("spin1y", 0.1, CFGF_NONE), CFG_FLOAT(
			"spin1z", 0.9, CFGF_NONE), CFG_FLOAT("spin2x", 0.11, CFGF_NONE), CFG_FLOAT("spin2y",
			0.15, CFGF_NONE), CFG_FLOAT("spin2z", 0.9, CFGF_NONE), CFG_FLOAT("qm1", 1, CFGF_NONE),
		CFG_FLOAT("qm2", 1, CFGF_NONE), CFG_FLOAT("incl", 1.4, CFGF_NONE),
		// mi a megfelelője?
		CFG_FLOAT("fI", 50, CFGF_NONE), CFG_FLOAT("fL", 2000, CFGF_NONE), CFG_FLOAT("fF", 20000,
			CFGF_NONE),
		// mi a megfelelője?
		CFG_FLOAT("d", 1, CFGF_NONE), CFG_FLOAT("dt", 6E-5, CFGF_NONE), CFG_STR("pnorder",
			"twoPointFivePN", CFGF_NONE), CFG_STR("spin", "QM", CFGF_NONE), CFG_END() };

	cfg_opt_t root[] = { CFG_SEC("waveDefault", params, CFGF_NONE), CFG_SEC("waveParams", params,
	...
	cfg_opt_t params_def[] = { CFG_FLOAT("m1", default_siminsp.mass1, CFGF_NONE), CFG_FLOAT("m2",
		default_siminsp.mass2, CFGF_NONE), CFG_FLOAT("spin1x", default_siminsp.spin1x, CFGF_NONE),
		CFG_FLOAT("spin1y", default_siminsp.spin1y, CFGF_NONE), CFG_FLOAT("spin1z",
			default_siminsp.spin1z, CFGF_NONE), CFG_FLOAT("spin2x", default_siminsp.spin2x,
			CFGF_NONE), CFG_FLOAT("spin2y", default_siminsp.spin2y, CFGF_NONE), CFG_FLOAT("spin2z",
			default_siminsp.spin2z, CFGF_NONE), CFG_FLOAT("incl", default_siminsp.inclination,
			CFGF_NONE), CFG_FLOAT("fI", 50, CFGF_NONE), CFG_FLOAT("fL", default_siminsp.f_lower,
			CFGF_NONE), CFG_FLOAT("fF", default_siminsp.f_final, CFGF_NONE), CFG_FLOAT("d", 1,
			CFGF_NONE), CFG_FLOAT("dt", default_ppn.deltaT, CFGF_NONE), CFG_STR("pnorder",
			"twoPointFivePN", CFGF_NONE), CFG_STR("spin", "QM", CFGF_NONE), CFG_END() };

	cfg_opt_t root_def[] = { CFG_SEC("waveDefault", params, CFGF_NONE), CFG_SEC("waveParams",
		params_def, CFGF_MULTI | CFGF_TITLE), CFG_END() };

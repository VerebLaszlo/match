...
void calculate_After(size_t index, detector det[], size_t *len) {
	...
	fftw_plan ipn = fftw_plan_dft_r2c_1d(det[*len].length, det[*len].cn, det[*len].n, FFTW_ESTIMATE);	// deallocated line: 71
	for(j = 0; j < det[*len].length; j++) {
		det[*len].ct[j][0] = det[p2].ct[j][0] * det[p1].c[j][0] + det[p2].ct[j][1] * det[p1].c[j][1];
		det[*len].ct[j][1] = det[p2].ct[j][0] * det[p1].c[j][1] - det[p2].ct[j][1] * det[p1].c[j][0];
		det[*len].cs[j][0] = det[p2].cs[j][0] * det[p1].c[j][0] + det[p2].cs[j][1] * det[p1].c[j][1];
		det[*len].cs[j][1] = det[p2].cs[j][0] * det[p1].c[j][1] - det[p2].cs[j][1] * det[p1].c[j][0];
		det[*len].cn[j][0] = det[p2].cn[j][0] * det[p1].c[j][0] + det[p2].cn[j][1] * det[p1].c[j][1];
		det[*len].cn[j][1] = det[p2].cn[j][0] * det[p1].c[j][1] - det[p2].cn[j][1] * det[p1].c[j][0];
	}
	fftw_execute(ipn);
...
int main(int argc, char *argv[]) {
	...
	for (i = 0; i < num_Detectors; i++) {
		...
		template[i].length = j;
		fclose(file);
		malloc_Waves(max_Length, &siganls[i]);										// deallocated line: end
	...
	detector *det = malloc(num_Detectors * sizeof(detector));						// deallocated line: end
	size_t length = fmin(fmin(templates[0].length, singals[0].length),noises[0].length);
	for (i = 1; i < num_Detectors; i++) {
		length = fmin(fmin(templates[i].length, singals[i].length), fmin(noises[i].length, length));
	}
	...
	for (i = 0, gen = 0, all_m = 0, bad_m = 0, good_m; ; i++) {
		...
		all_m++;
		length = fmin(fmin(templates[0].length, singals[0].length),noises[0].length);
		for (j = 1; j <= args; j++) {
			length = fmin(fmin(templates[j].length, singals[j].length), fmin(noises[j].length, length));
		}
		//	fill the detector structures
		...
		*norm = psd(det[num_Match].n, det[num_Match].length, dt, blackman);					// deallocated line: 396
		ts = scalar_freq(det[num_Match].ct, det[num_Match].cs, norm, minfr, maxfr);
		tt = scalar_freq(det[num_Match].ct, det[num_Match].ct, norm, minfr, maxfr);
		ss = scalar_freq(det[num_Match].cs, det[num_Match].cs, norm, minfr, maxfr);
		fftw_free(norm);
		num_Match++;
		...
		if (good_ == 1) {
			good_m++;
			write_Waveh(i, ht_in, length, dt);
			write_Wavel(i, lt_in, length, dt);
			write_Gen_Data(i, freq_Min, freq_Max, match, diff, good, &now, dt);
			write_Data_to_Plot(i, match, diff, good, &now);
		} else {
			bad_m++;
			write_Waveh(bad_m, ht_in, length, dt);
			write_Wavel(bad_m, lt_in, length, dt);

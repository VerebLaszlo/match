...
//	extern binary_system min, max;
	...
	type_name *d_names = malloc(num_Detectors * sizeof(type_name));	// deallocated at line: 93
	for (i = 0; !feof(file); i++) {
	...
	for (i = 0; i < num_Detectors; i++) {
		multi_Malloc(length, &det[i]);								// deallocated at line: 140
	...
	for (i = 0; i < num_Match - 1; i++) {
		double *norm = psd(det[i].n, det[i].length, dt, blackman);	// deallocated at line: 128
		...
		fftw_free(norm);											// allocated at line: 124
		if (i < num_Detectors) {
			ts_Sum += ts;
			tt_Sum += tt;
			ss_Sum += ss;
	...
	count[0] = 0;
	for (i = 0; i < num_Match - 1; i++) {
		multi_Free(det[i]);											// allocated at line: 85
	}
	// write the own match to the file
	file = sfopen_write("own.match");								// deallocated at line: 149
	for (i = 0; i < num_Match; i++) {
		fprintf(file, "%15.10lg ", own_Match[i]);
		printf("%15.10lg ", own_Match[i]);
	}
	printf("\n");
	fclose(file);													// allocated at line: 143
	// Initialize output files
	{
		file = sfopen_write("parameters.data");						// deallocated at line: 166
		...
		fclose(file);												// allocated at line: 152
		file = sfopen_write("to_Plot.data");						// deallocated at line: 173
		...
		fclose(file);												// allocated at line: 167
	}
	srand(time(NULL));
	// starting the loop
	...
	for (i = 1, gen = 0, all_m = 0, bad_m = 0, good_m = 0; ; i++) {
		...
		system(LALparams);
		errno = 0;
		file = fopen ("gen.out", "r");								// deallocated at line: 212
		if (file == NULL) {
		...
		double * fp, * fc;
		fp = malloc(num_Detectors * sizeof(double));				// deallocated at line: 213
		fc = malloc(num_Detectors * sizeof(double));				// deallocated at line: 214
...
//			fscanf(file, "%*g %lg %lg", &p, &c);
		...
		fclose(file);												// allocated at line: 188
		free(fp);													// allocated at line: 197
		free(fc);													// allocated at line: 198
		...
		//	fill the detector structures
		for (j = 0; j < num_Detectors; j++) {
			multi_Malloc(length, &det[j]);							// deallocated at line: 271
			for (k = 0; k < length; k++) {
		...
		for (j = 0; j < num_Match; j++) {
			double *norm = psd(det[j].n, det[j].length, dt, blackman);// deallocated at line: 261
			...
			fftw_free(norm);										// allocated at line: 257
			if (j < num_Detectors) {
				ts_Sum += ts;
				tt_Sum += tt;
				ss_Sum += ss;
		...
		for (j = 0; j < num_Match; j++) {
			multi_Free(det[j]);										// allocated at line: 226
		}
		for (j = 0; j < num_Match; j++) {
		...
		multi_Malloc(det[0].length * 2, &det[num_Match - 1]);		// deallocated at line: 314
		printf("Correlation: ");
		fflush(stdout);
		...
		double *norm = psd(det[num_Match - 1].n, det[num_Match - 1].length, dt, blackman);// deallocated at line: 314
		...
		fftw_free(norm);											// allocated at line: 309
		multi_Free(det[num_Match - 1]);								// allocated at line: 295
		num_Match++;

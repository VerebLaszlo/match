...
const DETECTOR_CONSTANTS DETECTOR_CONSTANT = { 630720013.0, 32.0, 189388800.0, 284083201.0,
	36525.0, 1.0 / 36525.0, 24110.54841, 8640184.812866, 0.093104, 6.2e-6, };
	...
	for (short i = 0; i < 3; i++) {
		...
		for (short j = i + 1; j < 3; j++) {
			response_Matrix[i][j] = response_Matrix[j][i] = (detector.direction_Vector_X[i]
					* detector.direction_Vector_X[j] - detector.direction_Vector_Y[i]
					* detector.direction_Vector_Y[j]) / 2.;
		}
	}
}
...
double convert_GMST_From_Seconds_To_Radians(double GPSsec) {
	...
	days_Since_Jan1st2000
			= floor(seconds_Since_Jan1st2000 * TIME_CONVERSION_CONSTANT.SECOND_TO_DAY) - 0.5;
	...
	mean_Sidereal_Time = DETECTOR_CONSTANT.COEFFICIENT_FOR_DEGREE0 + (centuries
			* (DETECTOR_CONSTANT.COEFFICIENT_FOR_DEGREE1 + centuries
					* (DETECTOR_CONSTANT.COEFFICIENT_FOR_DEGREE2 + centuries
							* DETECTOR_CONSTANT.COEFFICIENT_FOR_DEGREE3)));
	double coordinated_Universal_Time = mean_Sidereal_Time + seconds_In_Current_Day
			* 1.002737909350795; /* (UTC day is 1.002 * MST day) */
...
void print_Antenna_Function_Parameters(FILE *file, antenna_Func *antenna,
	...
	char format_String[4 * format->width_Of_Number_Width_Separator];
	set_Format_For(format_String, 4, format);
...
void print_Antenna_Function_Parameters_To_Plot(FILE*file, antenna_Func *antenna,
		OUTPUT_FORMAT_CONSTANTS *format) {
	char format_String[4 * format->width_Of_Number_To_Plot_Width_Separator];
	set_Plot_Format_For(format_String, 4, format);
	fprintf(file, format_String, antenna->polarization, antenna->declination,
			antenna->right_Ascention, antenna->gmst);
}
